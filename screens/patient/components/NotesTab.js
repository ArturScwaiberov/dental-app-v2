  ...
  return (
    <AppointmentCard>
      ...
      <AppointmentCardRow>
        ...
        />
        <AppointmentCardLabel>
          Doctor: <Bold>{clinicUsers.find((user) => user.id === item.createdBy)?.fullName}</Bold>
        </AppointmentCardLabel>
      </AppointmentCardRow>
...
const NotesTab = ({ notes, common, onFetch, onDeleteNote }) => {
  ...
  const renderItem = ({ item }) => <MemoizedListItem item={item} clinicUsers={clinicUsers} />
  ...
  return (
    <View style={{ backgroundColor: notes.length > 0 ? '#eee' : '#fff' }}>
      <SwipeListView
        data={notes}
