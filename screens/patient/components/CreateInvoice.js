...
const CreateInvoice = ({ patient, onClose, onUpdate }) => {
  ...
  const invoiceTotal = procedures.reduce((a, c) => a + c.qty * c.fee, 0).toFixed(2)
  ...
  const addProcedures = (newProcedures) => {
    const repeatedProcedures = newProcedures.filter((np) => procedures.some((p) => p.id === np.id))
    ...
    if (!repeatedProcedures.length) {
      ...
      setProcedures([
        ...
        ...procedures.map((p) => {
          const found = repeatedProcedures.find((rp) => rp.id === p.id)
          if (found) {
            return {
              ...p,
              qty: p.qty + found.qty,
  ...
  return (
    <ModalView>
      ...
      <Button block success onPress={createInvoice} disabled={!procedures.length}>
        <Text>Create Invoice - ${invoiceTotal}</Text>
      </Button>

      <Modal isVisible={isChooseProcedure} onBackdropPress={hideChooseProcedure}>
        <ChooseProcedure onClose={hideChooseProcedure} onAddProcedures={addProcedures} />
      </Modal>
    </ModalView>
  )
}
