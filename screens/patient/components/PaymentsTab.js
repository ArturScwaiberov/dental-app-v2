...
const ListItem = ({ item, createdBy, onCancel }) => {
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 0.7 }}>
        <Ionicons name='trash-outline' size={24} color='black' onPress={onCancel} />
      </View>
    </View>
  )
}
...
const PaymentsTab = ({ invoice, onAddPayment, onUpdate }) => {
  ...
  const fetchPayments = async () => {
    setLoading(true)
    const { data } = await proceduresApi.getInvoicePayments(token, patient.id, invoice.id)
    setPayments(
      data
  ...
  const renderItem = ({ item }) => (
    <MemoizedListItem
      item={item}
      createdBy={common.users.find((user) => user.id === item.createdCustomerId)?.fullName}
  ...
  const confirmPaymentCancellation = async () => {
    setLoading(true)
    hideConfirmDialog()
    await patientsApi.cancelPayment(token, patient.id, invoice.id, payment.id, {
      ...payment,
      isCanceled: true,
    })
    await onUpdate()
  ...
  return (
    <View style={{ padding: 16, flex: 1 }}>
      <Button block success onPress={showAddPayment} style={{ backgroundColor: '#2A86FF' }}>
      ...
      <Modal isVisible={isAddPayment} onBackdropPress={hideAddPayment}>
        <InvoicePaymentForm onClose={hideAddPayment} onAddPayment={onAddPayment} />
      </Modal>

      <Modal isVisible={!!payment} onBackdropPress={hideConfirmDialog}>
        <ConfirmDialog onClose={hideConfirmDialog} onConfirmation={confirmPaymentCancellation} />
      </Modal>
    </View>
  )
}
